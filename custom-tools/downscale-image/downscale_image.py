...
"""
Image Downscaler
Downscales images in a directory by a specified factor.
"""
...
def downscale_image(image_path, output_path, scale_factor):
    """
    Downscale a single image by the specified factor.
    """
    try:
        # Open image
        ...
        downscaled_img.save(output_path, quality=95)
...
def process_directory(input_dir, output_dir, scale_factor):
    """
    Process all images in the input directory and downscale them.
    """
    ...
    print(f"Found {len(image_files)} image file(s) to downscale...")
    print(f"Scale factor: 1/{scale_factor}x")
    ...
    for image_file in image_files:
        ...
        # Downscale image
        success, error = downscale_image(image_file, output_file, scale_factor)
        
        if success:
            print(f"  ✓ Downscaled to {output_file.name}")
            processed += 1
        else:
            print(f"  ✗ Error: {error}")
            failed += 1
...
def main():
    if len(sys.argv) < 4:
        print("Usage: downscale_image.py <input_dir> <output_dir> <scale_factor>")
        sys.exit(1)
