...
async function loadInstalledPlugins() {
  ...
  try {
    const plugins = await liteAPI.getPlugins();
...
window.deletePlugin = async function(filename) {
  ...
  try {
    await liteAPI.deletePlugin(filename);
    loadInstalledPlugins();
...
async function processBulkUpload() {
  ...
  for (const file of selectedFiles) {
    try {
      ...
      // Validate
      const validation = await liteAPI.validatePlugin(plugin);
      if (!validation.valid) {
  ...
  // Save all plugins
  try {
    const results = await liteAPI.bulkSavePlugins(plugins);
...
function setupUrlTab() {
  ...
  fetchBtn.addEventListener('click', async () => {
    ...
    try {
      const results = await liteAPI.bulkFetchPlugins(urls);
      ...
      if (plugins.length > 0) {
        ...
        const saveResults = await liteAPI.bulkSavePlugins(convertedPlugins);
...
function setupEditorTab() {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      await liteAPI.savePlugin(editorPluginData);
